...
| D5b | **Timing fallback transfer is balance-aware (lower Xᵣ with a starting balance)** | **Same as D5 but `billsAccountBalanceToday` = $500** (harness today `2030-01-01`, $100 weekly, next `2030-01-08`, $1,000 annual due `2030-01-20`, cushion $0 → 156 paychecks, `totalOutflow $3,000`) | `Xᵣ = min($100, ceilToCent((3000 + 0 − 500)/156)) = ceilToCent($16.0256) = `**`$16.03`** — strictly **less than** the $0-balance case's **$19.24** (D5), because the $500 balance is netted out. Startup catch-up `= requiredCatchUpForTransfer($16.03) = $800.83` (shown **$801**), lowest `-$800.82` on `2032-01-20` lifted to the `$0` cushion; yours `= $83.97/pay` (vs $80.76 at $0). Confirms a starting balance lowers the recurring transfer. |
